// Standalone Unit Test Runner for Media Usage Resolver Logic (Tests A through M)

const normalizeUrlConservatively = (url) => {
  if (!url) return '';
...
const evaluateFieldReference = (mediaItem, fieldValue, isHtml = false) => {
  if (!fieldValue || typeof fieldValue !== 'string') return null;
  const rawVal = fieldValue.trim();
  if (!rawVal) return null;

  if (rawVal === mediaItem.id) return 'media_id';
...
const queryUrl = 'https://cdn.icancam.vn/cms-media/news/2026/08/hero-B5GIR_KI.png?width=1200&quality=80#anchor';
const matchQuery = evaluateFieldReference(mediaA, queryUrl, false);
assert(matchQuery === 'normalized_url' || matchQuery === 'storage_path', 'Test G: URL with query-string parameters resolves accurately');

...
const syncFaqToSupabase = async (faq: FaqItem) => {
  try {
    const { error } = await supabase.from('faq_items').upsert({
      ...
      question_en: faq.questionEn || faq.questionVi,
      answer_vi: faq.answerVi,
      answer_en: faq.answerEn || faq.answerVi,
      status: faq.status || 'published',
      is_pinned: !!faq.isPinned,
      display_order: faq.displayOrder || 1,
      helpful_count: faq.helpfulCount || 0,
      unhelpful_count: faq.unhelpfulCount || 0,
      created_at: faq.createdAt || new Date().toISOString(),
      updated_at: faq.updatedAt || new Date().toISOString(),
